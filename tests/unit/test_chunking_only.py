#!/usr/bin/env python3
"""
Simple test script for RAG chunking functionality only.

Tests the sliding window chunking strategy without requiring
...
Machine learning continues to evolve with new techniques and applications emerging regularly. Companies like Google, OpenAI, and Microsoft are pushing the boundaries of what's possible with AI.
    """
...
def test_hybrid_search_metadata():
    ...
    tech_content = """
    The function `process_data()` handles JSON parsing and validation. It uses the pandas library for efficient data manipulation.
    ...
    For web development, React and Vue.js are popular JavaScript frameworks. Python's Django and Flask frameworks are widely used for backend development.

    The database query `SELECT * FROM users WHERE active = 1` retrieves all active users. Use parameters to prevent SQL injection attacks.
